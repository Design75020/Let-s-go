...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    try {
      setItems(JSON.parse(localStorage.getItem('cart_items') || '[]'));
      setRestaurantId(localStorage.getItem('cart_restaurant_id') || null);
      setRestaurantName(localStorage.getItem('cart_restaurant_name') || null);
      setDeliveryFee(parseFloat(localStorage.getItem('cart_delivery_fee') || '0'));
    } catch { /* noop */ }
    setHydrated(true);
  }, []);
  ...
  }, [items, restaurantId, restaurantName, deliveryFee, hydrated]);
